...
var global_counter, global_files: integer;
...
begin
  ...
  foreach var folder in folders do
    if ExtractFileName(folder) not in ['__pycache__', '.venv', '.idea', '.git', 'databases'] then 
  ...
  foreach var filePath in files do
    if ExtractFileName(filePath) not in ['.DS_Store', '.env', '.gitignore', 'README.md', 'requirements.txt', 'tasklist.md', 'TODO.md'] then
