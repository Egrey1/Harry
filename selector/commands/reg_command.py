...
class RegCoommand():
    ...
    async def send_role_picker(self, ctx: commands.Context) -> None:
        ...
        if not can_register(ctx.author) and not ctx.author.resolved_permissions.administrator:
            await ctx.interaction.followup.send('Подожди день, прежде чем ты сможешь снова зарегистрироваться')

        if not deps.game_state['game_started']:
            await ctx.interaction.followup.send('Дождитесь начала вайпа!', ephemeral= True)
            return None
        countries = await give_all_countries()  

        values = {i: i for i in countries if not deps.Country(i).busy}
        if not values:
            await ctx.interaction.followup.send("Увы, но доступных для регистрации стран нет", ephemeral= True)
            return None
