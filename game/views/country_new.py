...
class CountryNewView(View):
    ...
    async def return_new_button_callback(self, interaction: Interaction):
        ...
        (deps.PERSONAL['curpers'] in interaction.user.roles)):
            await interaction.response.send_message('У тебя нет на это прав!', ephemeral=True)
            return
        ...
        await country.send_news(embed.description)

        # Делаем кнопку неактивной в сообщении (потребуется редактирование сообщения при необходимости)
        try:
            interaction.message.components[0].disabled = True
        except Exception:
            pass
    ...
    async def otkat(self, interaction: Interaction, _: Button):
        ...
        embed = Embed(title=f'🔄 Откат новости от {interaction.user.mention}', description=interaction.message.content)
        ...
        await interaction.response.send_message(content=f'🔄 Эй-эй! {country_name}, видимо, что-то нарушил и поэтому модератор {interaction.user.mention} откатил эту новость')
