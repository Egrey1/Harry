...
class Country:
    ...
    def buy_factory(self, factory: 'Factory | str', quantity: int = -1):
        ...
        Params:
            factory (Factory | str): Завод или его имя, которое нужно купить. Если указано имя, то количество берется из quantity
            quantity (int): Обязательно указывается только если factory выбрано как его имя, иначе его можно использовать для более приоритетного выбора числа покупки фабрик
        
        Raises:
            ValueError: Ошибка возникает, когда factory.type() == str, quantity не определено
            ValueError: Ошибка возникает, когда у стране нет столько денег
            
        Note:
            Используемые БД:  
                - DATABASE_COUNTRY_PATH: для изменения состояния страны 
    ...
    def max_to_buy(self, factory: 'Factory | str') -> int:
        ...
        Params:
            factory (Factory | str): Фабрика или ее имя
        Returns:
...
class CommandPallete:
    ...
    ### Термины:
        - countryID: ID страны, для которой выполняется команда
        - stateID: ID региона, для которого выполняется команда

    ### Металингвистическая формула Бэкуса-Наура:
        - Алфавит: `= ; , : + (конкатенация) пробел countryID stateID ничего`
        - <команды>::= `<команда>` + [`пробел` + `<команда>`]
        - <команда>::= `<имя команды>` + `=` + `<параметры команды>`
        - <имя команды>::= `<строка>`
        - <строка>::= `<буква>` + `<строка>` | `ничего`
        - <буква>::= `a-z` | `A-Z` | `а-я` | `А-Я` | `0-9` | `пробел`
        - <параметры команды>::= ((`<имя атрибута>` + `:` + `<параметры>`) | `<параметры>`) + [`;` + `<параметры команды>`]
        - <параметры>::= `<параметр>` + [`,` + `<параметр>`]
        - <параметр>::= `<строка>` | `countryID` | `stateID`
        - <имя атрибута>::= `<строка>` | `countryID` | `stateID` | `ничего`
    ...
    # Доступные команды:
        ## annex_country:
            - **params**:
                ...
                - <параметр>: `countryID`
            - **context** (Country) 
            Страна, к которой присоединятся другие страны 

        ## annex_state:
            - **params**:
                - <имя атрибута>: `countryID` 
                - <параметр>: `stateID`
            - **context (None | Country)**
            если не None, то <имя атрибута> должен отсутствовать в первом <параметры команды>
    """
